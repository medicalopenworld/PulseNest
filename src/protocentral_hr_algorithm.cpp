...
signed long peakWindowHP[21], lastOnsetValueLED1, lastPeakValueLED1;
unsigned char HR[12];
unsigned char temp;
unsigned int lastPeak,lastOnset;
signed long movingWindowHP;
...
void hr_algo::initStatHRM (unsigned int sampling_freq) {
  unsigned char i;
...
void hr_algo::statHRMAlgo (signed long ppgData)
{
  unsigned char i;
  ...
  if (movingWindowCount >= movingWindowSize)
  {
    ...
    if (lastPeak>smallest)
    {
      ...
      if (ispeak==1)
      {
        ...
        // lastPeakValueLED1 = findMax(peakWindowHP); // ACM: redundant
        lastPeakValueLED1 = peakWindowHP[10]; // ACM fix
        totalFoundPeak++;
    ...
    if ((lastOnset>smallest)&&(ispeak==0))
    {
      ...
      {
        // values for SPO2 ratio
        // lastOnsetValueLED1 = findMin(peakWindowHP); // ACM: redundant
        lastOnsetValueLED1 = peakWindowHP[10]; // ACM fix
        totalFoundPeak++;
        if (totalFoundPeak>2)
        {
          // Update the HR and SPO2 buffer
    ...
    if (foundPeak>2)
    {
      // Every 3 new peaks update return values
      foundPeak=0;
      temp=chooseRate(HR);
      if ((temp>40)&&(temp<220))
        HeartRate=temp;
...
void hr_algo::updateWindow(signed long *peakWindow, signed long Y, unsigned char n)
{
...
signed long hr_algo::findMax(signed long *X)
{
...
signed long hr_algo::findMin (signed long *X)
{
