...
typedef struct afe44xx_Record {
  ...
  signed long ambientIR_data;
  signed long ambientRED_data;
  signed long IRminusAmbient_data;
  signed long REDminusAmbient_data;
  signed long IR_filtered_data;
  signed long RED_filtered_data;
...
public:
  ...
  void apply_bandpass_filter(afe44xx_data *afe44xx_raw_data);
...
private:
  ...
  BiquadState ir_filter_state = {0, 0};
  BiquadState red_filter_state = {0, 0};

  float step_biquad(float x, const float b0, const float b1, const float b2, const float a1, const float a2, BiquadState& state);
};
