...
{
  public:
	void initStatHRM (unsigned int sampling_freq);
  void statHRMAlgo (signed long ppgData);

  void updateWindow(signed long *peakWindow, signed long Y, unsigned char n);
  unsigned char chooseRate(unsigned char *rate);
  void updateHeartRate(unsigned char *rate, unsigned int freq, unsigned int last);
  signed long findMax(signed long *X);
  signed long findMin(signed long *X);
