...
from pulsenest_capture_csv import CaptureCsvWriter, LabCaptureWriter, CAPTURE_COLS  # noqa: F401
...
SIG_MAX_S          = 60    # PPGSignalsWindow internal accumulator: 60 s @ SPO2_RECEIVED_FS
...
class HWConfigWindow(QtWidgets.QMainWindow):
    ...
    def _setup_ui(self):
        ...
        self._combo_stg21.setToolTip(_make_tooltip("LED1 (IR) Stage 2 gain (STG2GAIN1)",
            "Stage 2 inverting amplifier gain for the IR channel. Implemented via feedback resistance RG — "
            "the kΩ values shown in the dropdown are the RG values (100 kΩ = ×1, 150 kΩ = ×1.5, etc.). "
        ...
        self._combo_stg22.setToolTip(_make_tooltip("LED2 (RED) Stage 2 gain (STG2GAIN2)",
            "Stage 2 inverting amplifier gain for the RED channel. Implemented via feedback resistance RG — "
            "the kΩ values shown in the dropdown are the RG values (100 kΩ = ×1, 150 kΩ = ×1.5, etc.). "
...
class PPGSignalsWindow(QtWidgets.QWidget):
    ...
    def __init__(self, main_monitor):
        ...
        self._sig_led2     = deque(maxlen=_sig_buf)
        self._sig_led1     = deque(maxlen=_sig_buf)
        self._sig_aled2    = deque(maxlen=_sig_buf)
        self._sig_aled1    = deque(maxlen=_sig_buf)
        self._sig_led2_sub = deque(maxlen=_sig_buf)
        self._sig_led1_sub = deque(maxlen=_sig_buf)
        self._sig_ppgdisp  = deque(maxlen=_sig_buf)
        self._sig_last_sc  = None   # last sample_counter value seen
        self._setup_ui()
        s = QtCore.QSettings(SETTINGS_FILE, QtCore.QSettings.IniFormat)
        ...
        self.spin_window_s.setValue(s.value("PPGSignalsWindow/window_s", int(WINDOW_SIZE / SPO2_RECEIVED_FS), type=int))
    ...
    def _setup_ui(self):
        ...
        self.spin_window_s.setToolTip(_make_tooltip(
            ...
            f"Range: 1\u2013{SIG_MAX_S} s. Uses an internal rolling buffer of "
            f"{int(SIG_MAX_S * SPO2_RECEIVED_FS)} samples "
            f"independent of the main {WINDOW_SIZE}-sample display buffer.",
    ...
    def update_plots(self, data_led2, data_led1, data_aled2, data_aled1,
        ...
        sc_list = list(data_sample_counter)
        if sc_list:
            last_sc = sc_list[-1]
            # Detect firmware restart (counter reset)
            if self._sig_last_sc is not None and last_sc < self._sig_last_sc:
                self._sig_last_sc = None
                for buf in (self._sig_led2, self._sig_led1, self._sig_aled2, self._sig_aled1,
                            self._sig_led2_sub, self._sig_led1_sub, self._sig_ppgdisp):
                    buf.clear()
            # Count entries strictly newer than last seen
            if self._sig_last_sc is None:
                n_new = len(sc_list)
            else:
                n_new = sum(1 for sc in reversed(sc_list) if sc > self._sig_last_sc)
            if n_new > 0:
                srcs = (data_led2, data_led1, data_aled2, data_aled1,
                        data_led2_sub, data_led1_sub, data_ppgdisp)
                dsts = (self._sig_led2, self._sig_led1, self._sig_aled2, self._sig_aled1,
                        self._sig_led2_sub, self._sig_led1_sub, self._sig_ppgdisp)
                for src, dst in zip(srcs, dsts):
                    dst.extend(list(src)[-n_new:])
        ...
        self.curve_led2.setData(tail(self._sig_led2))
        self.curve_led1.setData(tail(self._sig_led1))
        self.curve_aled2.setData(tail(self._sig_aled2))
        self.curve_aled1.setData(tail(self._sig_aled1))
        self.curve_led2_sub.setData(tail(self._sig_led2_sub))
        self.curve_led1_sub.setData(tail(self._sig_led1_sub))
        self.curve_ppgdisp.setData(tail(self._sig_ppgdisp))
...
class PPGSignals2Window(QtWidgets.QWidget):
    ...
    6 raw AFE4490 channels). Useful for cross-checking derived signals (OT, V_TIA,
    ...
    _ALL_SIGNALS = [
        ...
        ("V_TIA_LED1", "data_v_tia_led1"), ("V_TIA_LED2", "data_v_tia_led2"),
        ("V_TIA_ALED1", "data_v_tia_aled1"), ("V_TIA_ALED2", "data_v_tia_aled2"),
...
# LabCaptureWriter now lives in pulsenest_capture_csv.py (2026-09-20): tools/pulsenest_recorder.py
# writes the same CSV live in a hospital and cannot import this module, which pulls in Qt. R3 of
# capture_csv_format_spec.md asks for one writer shared by the lab, the recorder and the converter;
# the move is what makes that true. The name is re-exported there, so nothing else changed.
...
class LabCaptureWindow(QtWidgets.QMainWindow):
    ...
    # The canonical column table lives in pulsenest_capture_csv.py, so the recorder and the
    # converter index the frame exactly as this window does -- the one place where a mistake
    # is silent (a wrong index writes a well-formed row of garbage).
    _COLS = CAPTURE_COLS
...
class PPGMonitor(QtWidgets.QMainWindow):
    ...
    def __init__(self, save_chk=False, save_chk_duration=15):
        ...
        if save_chk:
            ...
            try:
                self.save_file_chk = open(self._chk_filename, "w", buffering=1, encoding="cp1252", errors="replace")
        ...
        self._STATS_SIGNALS = [
            ...
            ("LED1_SUB",   "data_led1_sub",   "Ambient-subtracted IR signal: LED1 − ALED1. Removes DC ambient component. Exposed for reference/diagnostics; HR1/HR2/HR3/SpO2 consume OT_LED1 instead (gain-invariant, lib v0.38+). Units: ADC counts.",    "AFE4490Data::led1_sub"),
            ("LED2_SUB",   "data_led2_sub",   "Ambient-subtracted RED signal: LED2 − ALED2. Removes DC ambient component. Exposed for reference/diagnostics; SpO2 consumes OT_LED2 instead (gain-invariant, lib v0.38+). Units: ADC counts.",         "AFE4490Data::led2_sub"),
            ("PPG_DISP",   "data_ppgdisp",    "Display-ready PPG signal, OT domain (lib v0.69): bandpass filter on OT_LED1/OT_LED2 → negated. Gain-invariant like the algorithms — no discontinuity when HGAC changes RF. NOT normalized for display (a candidate ppg_disp_norm, PI-weighted [0..1], is backlog). Units: A/A, tiny magnitude (~1e-5..1e-6).", "AFE4490Data::ppg_disp"),
            ...
            ("V_TIA_LED1",  "data_v_tia_led1",  "TIA DIFFERENTIAL output voltage, LED1/IR channel [V]. V_TIA = 2 × I_PD × RF (per-branch = /2). Computed per sample by firmware. Gray text = channel CLIPPED per CH_MASKS (ADC rail or TIA hard clip — value is a bound, not reality). Only available in $M4 frame mode.",                  "AFE4490DebugData::v_tia_led1"),
            ("V_TIA_LED2",  "data_v_tia_led2",  "TIA DIFFERENTIAL output voltage, LED2/RED channel [V]. Gray text = channel CLIPPED per CH_MASKS. Only available in $M4 frame mode.",                                                                      "AFE4490DebugData::v_tia_led2"),
            ("V_TIA_ALED1", "data_v_tia_aled1", "TIA DIFFERENTIAL output voltage, ALED1/IR ambient channel [V]. Gray text = channel CLIPPED per CH_MASKS. Only available in $M4 frame mode.",                                                             "AFE4490DebugData::v_tia_aled1"),
            ("V_TIA_ALED2", "data_v_tia_aled2", "TIA DIFFERENTIAL output voltage, ALED2/RED ambient channel [V]. Gray text = channel CLIPPED per CH_MASKS. Only available in $M4 frame mode.",                                                            "AFE4490DebugData::v_tia_aled2"),
            ("I_PD_LED1 [µA]",   "data_i_pd_led1",   "Photodiode current LED1/IR channel [µA]. I_PD = V_TIA / (2 × RF). Computed per sample by firmware. Gray text = channel CLIPPED per CH_MASKS (ADC rail or TIA hard clip — value is a bound, not reality). Only available in $M4 frame mode.",                "AFE4490DebugData::i_pd_led1"),
        ...
        self.btn_signals2.setToolTip(_make_tooltip(
            ...
            "3 slots selectable from ANY parsed $M1-$M4 signal (raw channels, OT, V_TIA, "
    ...
    def _update_stats_table(self):
        ...
        for sig_idx, (name, _, _tooltip, _src) in enumerate(self._STATS_SIGNALS):
            ...
            for col, v in enumerate(vals, start=4):
                ...
                # Analog-chain rows (V_TIA_*, I_PD_*, OT_*): gray text when the source
                # channel was CLIPPED during the stats window (value = bound, not reality)
                if 20 <= sig_idx <= 27:   # V_TIA_* (20-23) / I_PD_* (24-27): own channel
            ...
            if sig_idx in self._STATS_RAW_ROWS and buf:
                ...
                # → V_TIA = 2×I_PD×RF = 2 × (V_ADC/(2×RG) + I_CANCEL) × RI  (differential)
                v_tia = 2 * (v_adc / (2 * rg_ohm) + i_cancel) * ri_ohm
...
if __name__ == "__main__":
    ...
    class _TooltipDurationFilter(QtCore.QObject):
        """Event filter that gives every tooltip an indefinite display duration.

        Qt fires QEvent.ToolTipChange on a widget whenever setToolTip() is
        called on it. We intercept that event and pin the duration to INT_MAX
        (~24 days) so tooltips never auto-hide.
        """
        def eventFilter(self, obj, event):
            if (event.type() == QtCore.QEvent.ToolTipChange
                    and isinstance(obj, QtWidgets.QWidget)
                    and obj.toolTip()):
                obj.setToolTipDuration(2_147_483_647)
            return False  # never consume the event

    _ttf = _TooltipDurationFilter(app)
    app.installEventFilter(_ttf)
