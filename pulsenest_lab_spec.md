...
**Auxiliary sources (2026-09-20).** A source whose first datagram starts with a prefix in `pulsenest_hub.AUX_PREFIXES` — today `$VN1`, VideoNest's on-device OCR of the commercial monitor — is not a board: the hub forwards it like any other source but never asks it `$CFG?` (it has no configuration and does not listen on the command port), counts it apart in `@PONG` and shows it as `aux <kind>` in `@STATUS`; both fleet tools read the same table. Without this a phone appeared as a board with empty columns and took three `$CFG?` queries it could not answer. `fleet_ppg_viewer.py` gives it no band — a phone has no waveform. `fleet_monitor.py` gives it no row in the board table but a **`SOURCES (non-board)` block of its own** below it: kind, IP, rate, the SpO2 and confidence it is reading off the commercial monitor, its sequence number, its **`ID`** (the label the phone declares in the trailing field of `$VN1`, `-` when a frame carries none) and how long since it spoke; and it reaches the red alert line when it goes quiet. Excluding it from the board table was right (no MAC, no firmware, no RF: a row of dashes); excluding it from the screen was not — this tool answers *is everything alive?*, and a phone is the source most likely to stop. `source` is the word the rest of the project already uses: `pulsenest_recorder.py`'s `Source` class, `session.json`'s `sources` list.
